...
**Smart Analyzer** (`/analyzer`) is the live decision desk: STANDARD/SCALPING modes, score gauge, evidence chart with entry/SL/TP overlays, and **TAKE / Ignore** buttons that only record your choice (`POST /api/v1/signals/{id}/decision`) — never send an MT5 order.
