...
//| Read-only pending order capture                                  |
...
bool VantageLoadPendingOrders(const string symbol,
                              ...
                              VantagePendingOrderSummary &sum)
  {
   ...
   double mid = 0.0;
   if(px.bid > 0.0 && px.ask > 0.0)
      mid = (px.bid + px.ask) * 0.5;
   else if(px.bid > 0.0)
      mid = px.bid;
   else
      mid = px.ask;
   ...
   for(int i = 0; i < total; i++)
     {
      ...
      if(spec.point > 0.0)
         row.distance_points = row.distance_price / spec.point;
      else
      ...
      if(row.sl > 0.0 && row.volume > 0.0 && row.price_open > 0.0)
        {
         VantageRiskEstimate risk;
         ENUM_ORDER_TYPE ot = (ENUM_ORDER_TYPE)otype;
         if(VantageCalcRiskFromLevels(symbol, spec, ot, row.volume,
   ...
   for(int i = 0; i < sum.count; i++)
     {
      ...
      j += "\"distance_price\":" + DoubleToJson(sum.rows[i].distance_price, 8) + ",";
