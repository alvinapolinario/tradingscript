"""ICT bullish model — SSL sweep → displacement → MSS → FVG → BSL target."""
from __future__ import annotations

from app.analysis.ict.types import IctConfig, IctSetupContext, IctSetupState, LiquiditySweepEvent
...
def evaluate_bullish_sequence(
    ...
    sweep = ctx.sweep
    if not sweep or sweep.trade_bias != "BULLISH":
        return ctx

    post = [c for c in setup_candles if c.time >= sweep.sweep_time]
    if len(post) < 2:
        ctx.state = IctSetupState.WAITING_FOR_DISPLACEMENT
        ctx.reasons.append("Waiting for bullish displacement after SSL sweep.")
        return ctx
    ...
    if not bull_fvgs:
        ...
        ctx.reasons.append("Waiting for bullish FVG formation.")
        return ctx
...
def _apply_entry_state(ctx: IctSetupContext, price: float, cfg: IctConfig, atr_exec: float) -> None:
    ...
    if zone.zone_low <= price <= zone.zone_high:
        ...
        ctx.reasons.append("Price inside bullish FVG entry zone.")
        if ctx.trade_bias == "BULLISH":
            ctx.state = IctSetupState.TRIGGERED
            zone.status = "CONFIRMED"
    elif price > zone.zone_high + cfg.chase_max_atr * atr_exec:
        ctx.state = IctSetupState.EXPIRED
        ctx.invalidations.append(f"Price chased above entry zone {zone.zone_high:.2f}")
    else:
        ctx.state = IctSetupState.WAITING_FOR_RETRACE
        ctx.reasons.append("Waiting for retrace into bullish FVG.")
