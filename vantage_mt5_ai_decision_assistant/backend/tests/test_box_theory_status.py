...
def test_box_heartbeat_python_engine():
    """Heartbeat with box_candles runs Python canonical engine."""
    candles = []
    hi, lo = 3400.0, 3380.0
    mid = (hi + lo) / 2
    for i in range(30):
        if i % 4 == 0:
            candles.append({"time": 1000 + i * 900, "open": mid, "high": hi, "low": lo + 4, "close": hi - 0.5})
        elif i % 4 == 1:
            candles.append({"time": 1000 + i * 900, "open": mid, "high": hi - 4, "low": lo, "close": lo + 0.5})
        else:
            candles.append({"time": 1000 + i * 900, "open": mid, "high": hi - 2, "low": lo + 2, "close": mid})
    token = get_settings().local_api_token
    client = TestClient(app)
    r = client.post(
        "/api/v1/heartbeat",
        ...
        json={
            ...
            "bid": 3395.0,
            "digits": 2,
            "box_candles": {"H1": candles, "M15": candles, "M5": candles},
        },
    )
    assert r.status_code == 200
    monitor_store.select_symbol("XAUUSD")
    st = client.get("/api/v1/box-theory/status").json()
    assert st["box_theory_supported"] is True
    assert st.get("box_python_engine") is True
    assert st["box_theory"]["engine_source"] == "PYTHON_CANONICAL"
